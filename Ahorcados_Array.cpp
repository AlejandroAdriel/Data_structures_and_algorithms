...
template <int N>
bool AHORCADOS<N>::NUEVO_ESTADO(char letra)
{
    if (!HEAD)
    {
        HEAD = TAIL = ARR;
...
template <int N>
bool AHORCADOS<N>::RETOCEDER_ESTADO()
{
    if (!HEAD || NEstados <= 1)
        return false;
...
template <int N>
bool AHORCADOS<N>::LIBERAR_ESPACIO()
{
    if (!HEAD)
        return false;
